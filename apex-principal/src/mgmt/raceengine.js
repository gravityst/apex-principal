...
export function createWeekend(opts) {
  ...
  function fuelBurn(c, dtLaps) {
    ...
    // Never let fuel drop below what is still needed for the remaining distance.
    // This removes the possibility of running out of fuel entirely.
    const lapsLeft = totalLaps - c.lap;
    const minFuel = Math.max(0.5, lapsLeft * fuelPerLap * 0.15); // always keep a small safety margin
    c.fuel = Math.max(minFuel, c.fuel - burn);
    // No lift-and-coast penalty and no retirement for fuel.
    c.fuelSave = 0;
    // (Previously: if (c.fuel <= 0.001 && c.lap < totalLaps - 1) retire(c, 'out of fuel');)
  }

  // NOTE: The rest of the file is unchanged. Only fuelBurn was modified.
  // Because the full file is very large, we are updating only the critical
  // function via a targeted change. The surrounding code remains identical.
}
